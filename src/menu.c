...
void print_func(GtkWidget *widget, gpointer data)
{
	...
	//g_signal_connect(print, "draw-page", G_CALLBACK("draw-page"), NULL);
...
/*void print_func(GtkWidget *widget, gpointer data)
{
	...
	GtkPrinter *printer_selection;
	
	print_dialog = gtk_print_unix_dialog_new("Print Song In Display", GTK_WINDOW(window));
	printer_selection = gtk_print_unix_dialog_get_selected_printer(GTK_PRINT_UNIX_DIALOG(print_dialog));
